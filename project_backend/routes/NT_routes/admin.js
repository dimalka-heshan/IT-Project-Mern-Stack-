...
router.post('/add', async (req, res) => {
    try {
      const {fname, lname, email, username, password, nic, description, image} = req.body
  
      let admin1 = await admin.findOne({email})
      let admin2 = await admin.findOne({email})
      if (admin1 || admin2) {
...
//admin profile
  ...
  router.put('/update', auth, async (req, res) => {
    try {
      ...
      const adminUpdate = await admin.findByIdAndUpdate(req.admin1.id, 
        {fname : fname, lname : lname, email : email , username : username , password : hpassword , nic : nic ,description : description, image : image})
